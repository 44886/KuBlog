/*
 * Connect to the fireBase , Use ajax to load subPage, we use it once.
 * */
var cData =new Wilddog("https://github44886.wilddogio.com/");
...
$(function() {
	...
	/*
	 * Use ajax to load the subPage;
	 * */
	$.loadPage = function(pageName, saveToHistory) {
		if (pageName.indexOf("&") > -1) {
			var subPage = "subpages/" + pageName.substr(0, pageName.indexOf("&"));
		} else {
			var subPage = "subpages/" + pageName;
		}
		console.log(subPage);
		$.ajax({
			type: "get",
			...
			error: function() {
				$.AMUI.progress.done();
				$("#subPage").html($.errors("404"));
			}
		});
	};
	/*
	 * Do it ,when the mainPage be first visited.
	 * When someone visit the website by URL,check it ,get the subPage.
	 */
	var queryString = decodeURI(location.search.substr(1));
	if (!queryString || queryString == "") {
		queryString = "index.html";
	...
	/*
	 * when tag <a> be clicked.
	 * */
	$("body").on("click", "a", function() {
		/* If the new page needs a _blank target,do it.
		 */
		var target = $(this).attr("target");
		var url = $(this).attr("href");
		if (target == "_blank" || url == "#" || url == "javascript:void(0)" || url.substr(0, 7) == "http://" || url.substr(0, 8) == "https://") {
			return true;
		}
		if (url == "!") {
			return false;
		...
		/*
		 * If you want to give a var from this page to next page, use "data-push",the value is an Object.
		 */
		if ($(this).attr("data-push")) {
			eval('var push=' + $(this).attr("data-push") + ';');
			for (i in push) {
				window.localStorage[i] = push[i];
			}
		}
	...
	/* Someone click the "go-top" button.
	 */
	...
	$(window).scroll(function() {
		if ($(window).scrollTop() > 200 && $("#go-top:disabled")) {
			$("#go-top").fadeIn();
		} else {
			$("#go-top").fadeOut();
		}
	...
	$.checkLogin = function() {
		if (!cData.getAuth()) {
			$.loadPage("login.html", true);
		} else {
			console.log(cData.getAuth());
		}
		return false;
	}

	/*Show or hide the login btn*/
	if (cData.getAuth()) {
		$("#menu-login").hide();
		$("#menu-write").show();
	} else {
